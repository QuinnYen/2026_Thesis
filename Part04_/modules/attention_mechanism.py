...
class KeywordGuidedAttention(AttentionMechanism):
    ...
    def compute_attention(self, embeddings, metadata, **kwargs):
        ...
        if 'text' in metadata.columns:
            text_column = 'text'
        elif 'clean_text' in metadata.columns:
        ...
        for topic_idx, topic in enumerate(topics):
            ...
            if topic in topic_keywords:
                ...
                try:
                    ...
                    if len(parts) >= 2 and parts[0] == 'Topic':
                        topic_num = int(parts[1]) - 1
                        
                        # 查找對應編號的主題
                        for key in topic_keywords.keys():
                            if str(topic_num) in key or key.startswith(f"Topic_{topic_num+1}"):
            ...
            if not keywords:
                self.logger.warning(f"主題 {topic} 沒有找到關鍵詞，將使用均等權重")
                if doc_indices:
                    weights[topic_idx][doc_indices] = 1.0 / len(doc_indices)
                continue
...
def create_attention_mechanism(attention_type, config=None):
    """創建指定類型的注意力機制
...
def apply_attention_mechanism(attention_type, embeddings, metadata, 
                            topics_path=None, weights=None):
    """應用注意力機制計算面向向量
...
if __name__ == "__main__":
    ...
    doc_topics = np.random.choice(topics, size=n_docs)
    metadata = pd.DataFrame({
        'main_topic': doc_topics,
